...
const claudeData: CommandGroup[] = [
  {
    ...
    commands: [
      ...
      { command: "/compact [instructions]", description: "压缩当前对话历史以节省上下文窗口空间，可提供可选压缩指令", badge: "slash" },
      { command: "/init", description: "在当前项目目录初始化 CLAUDE.md 文件，包含项目结构和常用命令等上下文信息", badge: "slash" },
  ...
  },
  {
    ...
    commands: [
      ...
      { command: "/model", description: "切换当前会话使用的 AI 模型（如 claude-sonnet-4-5-20250929、claude-opus-4-20250514 等）", badge: "slash" },
      { command: "/allowed-tools", description: "管理允许 Claude 使用的工具列表，控制哪些工具可以在会话中被调用", badge: "slash" },
      { command: "/hooks", description: "管理 Claude Code 生命周期钩子，可在特定事件（工具调用前后）执行自定义脚本", badge: "slash" },
    ],
  },
  {
    category: "账户管理",
    icon: KeyRound,
    commands: [
      { command: "/login", description: "登录到 Anthropic 账户或切换到其他账户（支持 OAuth，Max Plan 用户可直接登录）", badge: "slash" },
  ...
  },
  {
    ...
    commands: [
      ...
      { command: "/add-dir", description: "将额外的工作目录添加到当前会话上下文中，让 Claude 可以访问更多文件", badge: "slash" },
    ],
  },
  {
    category: "其他命令",
    ...
    commands: [
      ...
      { command: "/status", description: "显示当前会话状态，包括账户、模型、上下文窗口使用情况等", badge: "slash" },
      { command: "/teleport", description: "将终端会话传送至 Claude 桌面应用或 Web，实现跨端无缝切换", badge: "slash" },
      { command: "/desktop", description: "切换到桌面应用进行可视化 Diff 审查，与终端会话共享同一 Claude Code 引擎", badge: "slash" },
    ],
  },
  {
    category: "键盘快捷键",
  ...
  },
  {
    ...
    commands: [
      ...
      { command: "claude -p / --print", description: "非交互打印模式（SDK 模式），处理完提示后直接退出，适合脚本和自动化", usage: 'cat logs.txt | claude -p "解释这些错误"', badge: "cli" },
      { command: "claude -c / --continue", description: "继续当前目录中最近一次的对话上下文，恢复之前的聊天记录", usage: "claude -c", badge: "cli" },
      { command: "claude -r / --resume [session]", description: "按 ID 或名称恢复指定的历史会话，可通过交互式菜单选择", usage: 'claude -r "auth-refactor" "继续这个 PR"', badge: "cli" },
      { command: "claude update", description: "更新 Claude Code 到最新版本（原生安装时自动后台更新，npm 安装时手动执行）", usage: "claude update", badge: "cli" },
  ...
  {
    category: "CLI 高级参数（Flags）",
    icon: Settings,
    commands: [
      { command: "--model", description: "设置当前会话使用的模型，支持别名（sonnet / opus）或完整模型名", usage: "claude --model claude-sonnet-4-5-20250929", badge: "flag" },
      { command: "--remote \"task\"", description: "在 claude.ai 上创建新的 Web 会话并指定任务描述，实现云端执行", usage: 'claude --remote "修复登录 Bug"', badge: "flag" },
      { command: "--teleport", description: "将 Web 会话恢复到本地终端继续执行", badge: "flag" },
      { command: "--chrome", description: "启用 Chrome 浏览器集成，用于 Web 自动化和测试", badge: "flag" },
      { command: "--agents '{...}'", description: "通过 JSON 动态定义自定义子代理，指定 description、prompt、tools、model 等字段", usage: `claude --agents '{"reviewer":{"description":"代码审查","prompt":"你是资深代码审查员","tools":["Read","Grep"],"model":"sonnet"}}'`, badge: "flag" },
      { command: "--from-pr [PR号/URL]", description: "恢复与指定 GitHub PR 关联的会话，方便继续 PR 相关的编码工作", usage: "claude --from-pr 123", badge: "flag" },
      { command: "--permission-mode", description: "设置权限模式", usage: "claude --permission-mode plan", badge: "flag",
        subcommands: [
          { command: "plan", description: "规划模式：Claude 制定计划但不执行，需用户确认" },
          { command: "default", description: "默认模式：根据操作类型提示权限" },
        ],
      },
      { command: "--tools \"Bash,Edit,Read\"", description: "限制 Claude 可用的工具集合（用空字符串禁用所有，或用 \"default\" 恢复全部）", badge: "flag" },
      { command: "--append-system-prompt", description: "在默认系统提示词末尾追加自定义文本，保留 Claude Code 原有能力", usage: "claude --append-system-prompt \"始终使用 TypeScript\"", badge: "flag" },
      { command: "--system-prompt", description: "完整替换系统提示词（慎用，会移除所有 Claude Code 内置指令）", badge: "flag" },
      { command: "--max-turns", description: "限制代理轮数（仅 print 模式），到达限制时退出并报错", usage: "claude -p --max-turns 5 \"query\"", badge: "flag" },
      { command: "--max-budget-usd", description: "设置 API 调用的最高费用上限（仅 print 模式）", usage: "claude -p --max-budget-usd 5.00 \"query\"", badge: "flag" },
      { command: "--output-format", description: "指定 print 模式的输出格式（text / json / stream-json）", usage: 'claude -p "query" --output-format json', badge: "flag" },
      { command: "--dangerously-skip-permissions", description: "跳过所有权限提示（高风险，仅在受信任的沙箱环境中使用）", badge: "flag" },
    ],
  },
];
...
const codexData: CommandGroup[] = [
  {
    ...
    icon: Play,
    commands: [
      { command: "codex", description: "启动 Codex CLI 交互式 TUI 会话（基于 Rust 重写，性能大幅提升）", badge: "cli" },
  ...
  },
  {
    ...
    icon: Cpu,
    commands: [
      { command: "--model / -m", description: "指定使用的 AI 模型。默认为 o4-mini，支持 gpt-5.3-codex 等最新 OpenAI 模型", usage: "codex --model gpt-5.3-codex", badge: "flag" },
      { command: "--provider / -p", description: "指定模型提供者。支持 openai（默认）、gemini、anthropic、ollama、mistral 等多种提供者", usage: "codex --provider anthropic --model claude-sonnet-4-5-20250929", badge: "flag" },
    ],
  },
  {
    category: "审批模式",
  ...
  },
  {
    ...
    commands: [
      ...
      { command: "/init", description: "生成项目的 AGENTS.md 配置文件，包含项目上下文和指令", badge: "interactive" },
  ...
  },
  {
    ...
    icon: Monitor,
    commands: [
      { command: "--image / -i", description: "将本地图片文件附加到提示中，支持多模态输入（远程图片 URL 跨 resume/backtrack 也可保留）", usage: 'codex -i screenshot.png "修复这个 UI 问题"', badge: "flag" },
  ...
  },
  {
    ...
    icon: Settings,
    commands: [
      { command: "~/.codex/config.toml", description: "Codex CLI 的全局配置文件（TOML 格式），设置默认模型、提供者和审批模式等", badge: "cli",
        subcommands: [
          { command: "model", description: "默认使用的模型名称（如 \"o4-mini\" 或 \"gpt-5.3-codex\"）" },
          ...
          { command: "model_supports_reasoning_summaries", description: "为自定义/未知模型启用推理摘要支持（不可用于禁用已知推理模型的推理功能）" },
        ],
      },
      { command: "AGENTS.md", description: "项目级配置文件，放在项目根目录。包含项目说明、技术栈、编码规范等上下文信息供 Codex 参考", badge: "cli" },
    ],
  },
];
...
const geminiData: CommandGroup[] = [
  {
    ...
    commands: [
      ...
      { command: "/compress", description: "压缩当前对话上下文以释放 Token 空间，保留关键信息", badge: "slash" },
      { command: "/rewind", description: "撤回最近一轮对话（包括用户输入和模型回复），回到上一个状态继续操作", badge: "slash" },
  ...
  },
  {
    ...
    icon: Play,
    commands: [
      { command: "gemini", description: "启动 Gemini CLI 交互式会话（基于 Google Gemini 3 模型，免费层 60 次/分钟）", badge: "cli" },
      { command: "--model / -m", description: "指定使用的 Gemini 模型（如 gemini-2.5-flash、gemini-2.5-pro 等，默认使用 Gemini 3）", usage: "gemini -m gemini-2.5-flash", badge: "flag" },
      ...
      { command: "--output-format", description: "指定输出格式（text / json / stream-json），json 格式便于脚本解析", usage: 'gemini -p "query" --output-format json', badge: "flag" },
      { command: "--include-directories", description: "指定额外目录作为工作上下文（逗号分隔），让 Gemini 可访问多个目录", usage: "gemini --include-directories ../lib,../docs", badge: "flag" },
